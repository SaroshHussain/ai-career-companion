...
import { MdArticle, MdDescription, MdRecordVoiceOver, MdWork } from 'react-icons/md'
    ...
    <section id="features" className="mx-auto max-w-container-max px-margin-mobile py-3xl md:px-gutter">
      <div className="mb-2xl text-center">
        <div className="mb-md inline-flex items-center gap-2 rounded-full bg-primary/10 px-4 py-1.5 text-primary">
          ...
          <span className="text-label-sm uppercase tracking-wider">Features</span>
        </div>
        ...
        {features.map((feature) => {
          ...
          if (feature.variant === 'resume') {
            return (
              <Reveal key={feature.title} className="md:col-span-7">
                <article
                  className="flex h-full cursor-pointer flex-col justify-between rounded-lg border border-outline-variant bg-surface-container-lowest p-xl shadow-card transition-all duration-200 hover:-translate-y-1 hover:shadow-card-hover"
                ...
                >
                  ...
                  <div className="mt-xl flex items-center justify-between border-t border-outline-variant/30 pt-xl">
                    ...
                    <a
                      href="#"
                      className="text-label-md text-primary transition hover:underline"
                      onClick={(event) => {
                        event.preventDefault()
          ...
          if (feature.variant === 'interview') {
            return (
              <Reveal key={feature.title} delay={100} className="md:col-span-5">
                <article
                  className="flex h-full cursor-pointer flex-col rounded-lg border border-outline-variant bg-surface-container-high p-xl shadow-card transition-all duration-200 hover:-translate-y-1 hover:shadow-card-hover"
                  onClick={handleProtectedNavigation}
                  onKeyDown={handleKeyDown}
                  tabIndex={0}
                  role="button"
          ...
          if (feature.variant === 'matching') {
            return (
              <Reveal key={feature.title} delay={150} className="md:col-span-5">
                <article
                  className="flex h-full cursor-pointer flex-col rounded-lg border border-outline-variant bg-surface-container-lowest p-xl shadow-card transition-all duration-200 hover:-translate-y-1 hover:shadow-card-hover"
                  onClick={handleProtectedNavigation}
                  onKeyDown={handleKeyDown}
                  tabIndex={0}
                  role="button"
          ...
          return (
            <Reveal key={feature.title} delay={200} className="md:col-span-7">
              <article
                className="relative flex h-full cursor-pointer flex-col overflow-hidden rounded-lg border border-primary bg-primary-container p-xl text-white shadow-card transition-all duration-200 hover:-translate-y-1 hover:shadow-card-hover"
                onClick={handleProtectedNavigation}
                onKeyDown={handleKeyDown}
                tabIndex={0}
                role="button"
